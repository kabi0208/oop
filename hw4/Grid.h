...
class Grid {
    ...
    private:
        char grid[42][42];
        char tmp; // use to store character when place block
        int random; // use for choose the exit
        int row, column; // row, column of grid + 2 (include width of blocks)
        const char direction[4]={'^', '<', 'v', '>'};
        int button; // 1 for ON, 0 for off;
        int mover_r, mover_c, mover_d; // mover_r : current row of mover, mover_c : current column of mover, mover_d : current direction of mover
        int i, j; // use for any calculation
};